...
public class MancalaBoardStandard implements MancalaBoardFormatter {
	...
	// Copy-constructor
	public MancalaBoardStandard(MancalaBoardStandard boardStandard) {
		boardPanel = boardStandard.boardPanel;
		board = boardStandard.board;
	}
	...
	public MancalaBoardFormatter cloneThis() {
		return new MancalaBoardStandard(this);
	}
	...
	public void createShapes() {
		...
		boardPanel.getPitGraphicsMap().get(Pit.MANCALA_B).setOuterBound(new RoundRectangle2D.Double(
				board.getX() + pitPad, board.getY() + pitPad, pitWidth, bigPitHeight, pitWidth, pitWidth));
		...
		((RoundRectangle2D) board).setRoundRect(pad, pad, WIDTH - pad * 2, HEIGHT - pad * 2, pitWidth + pad,
				pitWidth + pad);
